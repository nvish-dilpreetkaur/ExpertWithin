      ...
      <div class="col-md-12">
        <div class="inner-oppor-wrapper">
          <div class="col-md-12 inner-wrapper">
            <div class="row clearfix">
              <div class="col-md-12">
                <h1 class="opportunity-heading">{{ __('Favorites') }}</h1>
              </div>
            </div>
          </div>
          <table class="table">
						...
						@foreach($myfavorites as $key => $myfavorite)
						 <tr>
							...
							<td>
								 ...
								 <h5><a href="{{ url('view-opportunity', Crypt::encrypt($myfavorite->id)) }}">{{ $myfavorite->opportunity }}</a></h5>
								 <h6>
							...
							<td class="text-left"> {{ $myfavorite->opp_manager }}</td>
							<td class="text-left"> {{ date('m-d-y', strtotime($myfavorite->start_date)) }}</td>
							<td class="text-left"> {{ date('m-d-y', strtotime($myfavorite->end_date)) }}</td>
							<td>  
								<div class="oppor-detail for-favorite">
                   <a href=" {{ url('opportunity/not_favourite', Crypt::encrypt($myfavorite->id)) }}"><i class="fa fa-heart" aria-hidden="true"></i></a>
								  </div>
							</td>
