...
<!-- filters html -->
	...
	<div class="recently-viewed-section-outer">
		<div class="container">
			<div class="recently-viewed-oppor-section view-oppor-section @if ($activityOpportunities == null)  no-act-found @endif">
			<h2>My Activities</h2>	
			@if ($activityOpportunities != null)
			<div class="row clearfix">
				@php
				 	$loopCounter2 = 0; 
					$home_card_cls = home_card_cls(); 

					$focus_list_data = org_focus_list();   
	 				$focusList = $focus_list_data['focusList']; 
				@endphp
				@foreach($activityOpportunities as $likeKey => $likeRow)
					...
					<div class="card-common">
						<div class="card-hdng">
							<div class="card-title-area {{ $home_card_cls[$loopCounter2][0] }}">
								...
								$skillcounter = count(explode(',', $likeRow->focus_areas));
								$hiddenSkillCount = $skillcounter-1;
								@endphp
								<img src="{{ URL::asset('images') }}/{{ $home_card_cls[$loopCounter2][1] }}" class="card-book-pic">
								<div class="{{ $home_card_cls[$loopCounter2][2] }}" @if ( $hiddenSkillCount > 0 ) title="Focus Areas" data-toggle="popover" data-content="@foreach(explode(',', $likeRow->focus_areas) as  $focus_area)
										{{ $loop->first ? '' : ', ' }}
										{{$focusList[$focus_area]}}
									@endforeach " @endif>
									
									@foreach(explode(',', $likeRow->focus_areas) as  $focus_area)
										{{ $loop->first ? $focusList[$focus_area] : '' }}
									@endforeach

									@if ( $hiddenSkillCount > 0 )
									<a href="javascript:void(0)" class="hiddenSkillCount">{{ '+'.$hiddenSkillCount }}</a>
									@endif
								</div>
						...
						<h3><a href="{{ url('view-opportunity', Crypt::encrypt($likeRow->id)) }}">{{ char_trim($likeRow->opportunity,28) }}</a></h3>
						...
						<div class="card-footer">
							<div class="card-like common-card-footer" id="likeBtn{{$likeRow->id}}wrapper" data-likeurl="{{ url('opportunity/like', Crypt::encrypt($likeRow->id)) }}" data-unlikeurl="{{ url('opportunity/not_like', Crypt::encrypt($likeRow->id)) }}" data-likeimgurl="{{ URL::asset('images/thumbs-up-r.svg') }}" data-unlikeimgurl="{{ URL::asset('images/thumbs-up.svg') }}" data-page="my-activity">
								<!-- mark like -->
								@if(!$loggedInUserID)
