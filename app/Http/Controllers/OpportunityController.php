...
class OpportunityController extends Controller
{
  ...
  function view($id = null){
    ...
    if ($id) {
        ...
        $oppData = $this->opportunity->getOpportunityOneDetails($filters);
        $opportunity_data = $oppData;

        $filter1['skills'] = $opportunity_data->skill_id;
        $filter1['oid'] = $opportunity_data->id;
        $opp_skill_list = $this->opportunity->getOpportunityRelatedSkills($filter1);

        $filter2['skills'] = $opportunity_data->focus_areas;
        $filter2['oid'] = $opportunity_data->id;
        $opp_focus_list = $this->opportunity->getOpportunityRelatedSkills($filter2);
      
        /** you may also like : start */
        $filter2['loggedUserID'] = $loggedInUserID;
        $opp_you_may_like = $this->opportunity->getOpportunityYouMayLike($filter2);
         /** you may also like : end */

        /** mark as viewed code: start*/
        $this->opportunity->markOppViewed($id);
        /** mark as viewed code: end*/

         /** get opp public comments: start*/
         $filters['comment_type'] = config('kloves.COMMENT_TYPE_PUBLIC');
         $filters['comment_status'] = config('kloves.COMMENT_ACTIVE');
         $commentList = $this->opportunity->getOppCommentList($id,$filters);
         /** get opp public comments: end*/

          /** get applicant list : start*/
          $appfilter['id'] = $id;
          $appfilter['loggedUserID'] = $loggedInUserID;
          $appfilter['loggedUserRole'] = $roleId;
          $applicantData = $this->opportunity->getOpportunityApplicantList($appfilter); //dd( $applicantData);
          /** get applicant list: end*/
      }
    
    $page_title = "Opportunity Details";
    $home_card_cls = config('kloves.HOME_CARD_CLASSES'); //dd($home_card_cls);
   
    $topMatchedOpportunities = $this->opportunity->getTopMatchedOpportunities($topMatchedFilter);
    $focusList = [];
    return view('opportunity.view', compact(['opportunity_data','opp_skill_list','opp_you_may_like','page_title','opp_focus_list','loggedInUserID','home_card_cls','topMatchedOpportunities','commentList','applicantData']));
  }
   /**
	 * Show the opportunity applicants.
   * AJAX Call
	 * @param  id
	 * @return Response
	*/
  function applicants($id = null){ 
      $response = array( 
        "type" => NULL,
        "html" => NULL,  
      );
      if ($id) {
        ...
        $user = \Auth::user();  
        if(!empty($user)){
          $filters['loggedUserID'] = $user->id;
          $filters['loggedUserRole'] = $user->role;
        }
        $opportunity_data = DB::table('org_opportunity')->where('id',$id)->first();
        $filters['id'] = $id;
        $applicantData = $this->opportunity->getOpportunityApplicantList($filters);

        $response["html"] = view("opportunity.oppo_applicant_list")
				->with("applicantData", $applicantData)
				->with("opportunity_data", $opportunity_data)
        ->render();
        
			  $response["type"] = "success"; // dd($response);	
      }else{
        $response["type"] = "error";	
      }
   ...
   /**
	 * Show the opportunity public comments.
   * AJAX Call
	 * @param Request
	 * @return Response
	*/
  function getOppPublicComments(Request $request){
    $response = array( 
      "type" => NULL,
      "html" => NULL,  
    );
    if ($request->ajax()) {   
      ...
      $response["html"] = view("opportunity.comment_listing")
   ...
   /**
	 * Show the opportunity user comments.
   * AJAX Call
	 * @param Request
	 * @return Response
	*/
  function getOppConversation(Request $request){
    $response = array( 
      "type" => NULL,
      "html" => NULL,  
    );  
    if ($request->ajax()) {   
      $id = Crypt::decrypt($request->post('oid'));
      $filters['applicant_id'] = Crypt::decrypt($request->post('applicant_id'));
      $filters['comment_type'] = config('kloves.COMMENT_TYPE_PRIVATE');
      $filters['comment_status'] = config('kloves.COMMENT_ACTIVE'); 
      $filters['limit'] = 3; 
      $pvtcommentList = $this->opportunity->getOppCommentList($id,$filters);
      //dd($pvtcommentList);
      $response["html"] = view("opportunity.post-comment") 
      ->with("pvtcommentList", $pvtcommentList)
      ->render();
